...
function browserRollup() {
  return rollup({
    ...
    external: [
      ...
      "@capnp-js/trans-stream",
    ],
  }).then(bundle => {
    bundle.write({
      file: "browser/capnp-js-reader-arena.js",
      format: "umd",
      ...
      globals: {
        ...
        "@capnp-js/trans-base64": "capnpJsTransBase64",
        "@capnp-js/trans-packing": "capnpJsTransPacking",
        "@capnp-js/trans-stream": "capnpJsTransStream",
      },
    });
  });
}
...
function browserUglify() {
  return new Promise((resolve, reject) => {
    fs.readFile("browser/capnp-js-reader-arena.js.map", (err, map) => {
      if (err) {
        reject(err);
      } else {
        const ugly = gulp.src("browser/capnp-js-reader-arena.js")
          .pipe(uglify({
            sourceMap: {
              content: map.toString(),
              url: "capnp-js-reader-arena.js.map",
            },
          }))
